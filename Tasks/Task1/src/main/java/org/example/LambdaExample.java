...
public class LambdaExample {
    ...
    public static void main(final String[] args) {
        ...
        repository.select(ageGreaterThan5, nameComparator);

        banner("Listing users with age > 5 sorted by name - lambda");
        // SOLVED With functional interfaces used directly
        repository.select(user -> user.age > 5, Comparator.comparing(u -> u.name));

        banner("Listing users with age < 10 sorted by age");
        // SOLVED With functional interfaces declared
        Predicate<User> ageSmallerThan10 = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age < 10;
            }
        ...
        repository.select(ageSmallerThan10, ageComparator);

        banner("Listing users with age < 10 sorted by age - lambda");
        // SOLVED With functional interfaces used directly
        repository.select(user -> user.age < 10, Comparator.comparingInt(u -> u.age));
        ...
        // SOLVED With functional interfaces used directly
        repository.select(user -> user.active, Comparator.comparing(u -> u.name));

        banner("Listing active users with age > 8 sorted by name");
        // SOLVED With functional interfaces declared
        Predicate<User> greaterThan8AndActive = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age > 8 && user.active;
            }
        };
        repository.select(greaterThan8AndActive, nameComparator);

        banner("Listing active users with age > 8 sorted by name - lambda");
        // SOLVED With functional interfaces used directly
        repository.select(user -> user.age > 8 && user.active, Comparator.comparing(u -> u.name));
    }
